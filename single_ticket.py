class SalesforceTicket():
    ...
    def get_owner_ticket_list(self, owner_id: str):
        ...
        if isinstance(ticket_list_response, list) and ticket_list_response:
            if ticket_list_response[0]['Type'] != entryTypes['error']:
                return ticket_list_response[0].get('Contents', [])
            else:
                return_error("No ticket list has been returned")
        return []

    def set_ticket_number(self, ticket_number: int):
        self._ticket_number = ticket_number  # Update the attribute with the new value

    def set_ticket_status(self, ticket_status: str):
        self._ticket_status = ticket_status  # Update the attribute with the new value
    ...
    def get_ticket_acv(self, ticket_creation_date):
        ...
        if isinstance(acv_query_response, list) and acv_query_response:
            if acv_query_response[0]['Type'] != entryTypes['error']:
                ...
                if contents:  # Check if contents is not empty
                    ...
                    # Append to table data only if _ticket_number is set
                    table_data.append({
        ...
        return []  # Return an empty list if there was an error or no contents
...
def main():
    ...
    salesforce_ticket_instance = SalesforceTicket()  # Define class instance

    owner_id = salesforce_ticket_instance.get_owner_id(full_name)

    if not owner_id:
        return_error(f"No owner ID found for the full name: {full_name}")

    # Get the ticket list by OwnerId
    ticket_list_content = salesforce_ticket_instance.get_owner_ticket_list(owner_id)

    if not ticket_list_content:
        return_error(f"No tickets found for owner ID: {owner_id}")
    ...
    for item in ticket_list_content:
        ...
        if ticket_number:
            ...
            acv_data = salesforce_ticket_instance.get_ticket_acv(ticket_creation_date)  # get acv + duplicates and populate a list
